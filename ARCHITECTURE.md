...
В проекте основная сложность связана не с вычислениями, а с изменяемостью правил: появляются новые условия, меняются целевые URL, добавляются ссылки, отличаются окружения запуска. Поэтому архитектура направлена на снижение стоимости доработок без переписывания основного кода сервисов.
